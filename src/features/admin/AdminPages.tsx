...
export function ReportsPage() {
  ...
  const operationsQuery = useQuery({
    queryKey: queryKeys.reports("operations", query),
    queryFn: () => tmsService.reportsOperations(query),
  });
  const billingQuery = useQuery({
    queryKey: queryKeys.reports("billing", query),
    queryFn: () => tmsService.reportsBilling(query),
  });
  ...
  const chartValues = normalizeBars(
    orderGroups.map((group) => groupCount(group)),
  );
  ...
  return (
    <div>
      <PageHeader
        title="Reportes"
        subtitle="Rendimiento operativo, ingresos y SLA"
      />
      {(operationsQuery.isLoading || billingQuery.isLoading) && (
        <div className="inline-alert inline-alert--info">
          Generando reportes desde transport-api...
        </div>
      )}
      {(operationsQuery.isError || billingQuery.isError) && (
        <div className="inline-alert">No se pudieron cargar los reportes.</div>
      )}
      ...
      <section className="reports-layout">
          ...
          <div className="bar-chart">
            {chartValues.map((value, index) => (
              <div key={index}>
                ...
                <small>
                  {String(orderGroups[index]?.status ?? "--").slice(0, 3)}
                </small>
              </div>
            ))}
          </div>
        ...
        <aside className="panel export-panel">
          <div className="panel-heading">
            <div>
              <h2>Exportar</h2>
              <p>CSV, Excel y PDF desde API</p>
            </div>
          </div>
          {exportButtons.map((item) => (
            <button
              key={`${item.type}-${item.format}`}
              onClick={() => void exportReport(item.type, item.format)}
            >
              <Download size={16} />
      ...
      <section className="panel indicators">
        <div className="panel-heading">
          <div>
            ...
            <p>Comparación del filtro actual</p>
          </div>
        </div>
        <div>
          <span>Entregas</span>
          <strong>{delivered} completadas</strong>
          <em className="positive">{totalOrders} órdenes totales</em>
          <StatusBadge>API</StatusBadge>
        </div>
        <div>
          <span>Evidencias</span>
          <strong>{sumCount(proofGroups)} registros</strong>
          <em>Validación por estado</em>
...
export function AuditPage() {
  ...
  const selectedLog = logs.find(
    (log) => String(log.id) === String(selected?.id),
  );
  ...
  return (
    <div>
      <PageHeader
        title="Auditoría"
        subtitle="Trazabilidad de cambios y eventos críticos"
        onExport={exportAudit}
      />
      {auditQuery.isError && (
        <div className="inline-alert">
          No se pudo cargar auditoría desde la API.
        </div>
      )}
      ...
      <section className="stats-grid">
        ...
        <StatCard
          stat={{
            label: "Entidades",
            value: String(new Set(rows.map((row) => row.entidad)).size),
            helper: "Con actividad",
            tone: "green",
          }}
        />
        <StatCard
          stat={{
            label: "Actores",
            value: String(new Set(rows.map((row) => row.actor)).size),
            helper: "Usuarios/Sistema",
            tone: "slate",
          }}
        />
        <StatCard
          stat={{
            label: "IPs",
            value: String(new Set(rows.map((row) => row.ip)).size),
            helper: "Origen de cambios",
            tone: "red",
          }}
        />
      </section>
      ...
      <section className="audit-layout">
        <article className="panel audit-list">
          <div className="panel-heading">
            <div>
              ...
              <p>Todos los eventos vienen de /audit</p>
            </div>
          </div>
          {rows.map((event) => (
            <button
            ...
            >
              <time>{String(event.hora)}</time>
              <span>
                <strong>{String(event.accion)}</strong>
                <small>
                  {String(event.actor)} · {String(event.entidad)}
                </small>
              </span>
              <ChevronRight size={15} />
            </button>
        ...
        <aside className="dark-insight audit-change">
          <span>Cambio seleccionado</span>
          <h3>{String(selected?.accion ?? "Sin evento")}</h3>
          <p>
            {String(selected?.actor ?? "Sistema")} ·{" "}
            {String(selected?.entidad ?? "--")}
          </p>
          <div>
            <span>Entidad ID</span>
            <strong>{String(selected?.entityId ?? "--")}</strong>
          </div>
          <div>
            <span>IP</span>
            <strong>{String(selected?.ip ?? "--")}</strong>
          </div>
          <div>
            <span>Payload</span>
            <strong>{selectedLog ? payloadSize(selectedLog) : "--"}</strong>
          </div>
          <StatusBadge>Auditado</StatusBadge>
        </aside>
      </section>
    </div>
  );
...
const settingsCards = [
  {
    ...
    copy: "Despacho, auditoría, finanzas",
    status: "roles",
  },
  {
    icon: Map,
    title: "Mapas y GPS",
    copy: "Google Maps · tracking",
    status: "parameters",
  },
  {
    icon: Bell,
    title: "Notificaciones",
    copy: "Push, correo operativo",
    status: "catalogs",
  },
  {
    icon: Cloud,
    title: "Almacenamiento",
    copy: "Evidencias y documentos",
    status: "parameters",
  },
  {
    icon: PlugZap,
    title: "Integraciones",
    copy: "Pagos, webhooks y APIs externas",
    status: "catalogs",
  },
  {
    icon: KeyRound,
    title: "Seguridad y API Keys",
    copy: "Credenciales y rotación",
    status: "parameters",
  },
];
...
export function SettingsPage() {
  ...
  const usersQuery = useQuery({
    ...
    queryFn: () => tmsService.users(query),
  });
  const rolesQuery = useQuery({
    queryKey: ["roles"],
    queryFn: () => tmsService.roles(),
  });
  const parametersQuery = useQuery({
    queryKey: queryKeys.parameters(),
    queryFn: () => tmsService.parameters(),
  });
  const catalogsQuery = useQuery({
    queryKey: queryKeys.catalogs(),
    queryFn: () => tmsService.catalogs(),
  });
  ...
  return (
    <div>
      <PageHeader
        title="Configuración"
        subtitle="Roles, integraciones y reglas del sistema"
      ...
      {(usersQuery.isError ||
        ...
        <div className="inline-alert">
          No se pudo cargar toda la configuración desde la API.
        </div>
      )}
      <section className="stats-grid">
        <StatCard
          stat={{
            ...
            helper: `${countRows(users, "ACTIVE")} activos`,
            tone: "blue",
          }}
        />
        <StatCard
        ...
        <article className="panel settings-panel">
          <div className="panel-heading">
            <div>
              ...
              <p>Administración del entorno productivo</p>
            </div>
          </div>
          {settingsCards.map((item) => (
            <button
              key={item.title}
              onClick={() =>
                toast.info(`${item.title} sincronizado con ${item.status}`)
              }
            >
        ...
        <aside className="system-health">
          <div className="panel-heading">
            <div>
              ...
              <p>Datos consultados desde API</p>
            </div>
            <Activity size={20} />
          </div>
          <div className="health-score">
            <strong>{usersQuery.isSuccess ? "OK" : "--"}</strong>
            <span>API principal</span>
          </div>
          <p>
            <i className="green-dot" /> Configuración consultada vía JWT
          </p>
          <div>
            <span>Usuarios</span>
        ...
        <DataTable
          ...
          onEdit={(row) =>
            tmsService
              .updateUserStatus(
                String(row.id),
                row.estado === "ACTIVE" ? "INACTIVE" : "ACTIVE",
              )
              .then(() =>
                queryClient.invalidateQueries({ queryKey: ["users"] }),
              )
              .then(() => toast.success("Estado de usuario actualizado"))
              .catch((error: unknown) =>
                toast.error(
          ...
          onDelete={() =>
            toast.info("Desactivar usuario usa Editar para mantener auditoría.")
          }
        />
      </div>
      <Modal
        ...
        title="Crear usuario"
        description="Asigna roles y estado inicial de cuenta."
        wide
      >
        <EntityForm
          fields={fields}
...
function groupCount(group: AnyRecord) {
  ...
  return typeof count === "object" && count && "_all" in count
    ? Number(count._all)
    : 0;
}
...
function countByGroup(groups: AnyRecord[], field: string, value: string) {
  return groups
    ...
    .reduce((total, group) => total + groupCount(group), 0);
}
...
function sumAmount(groups: AnyRecord[]) {
  return groups.reduce((total, group) => {
    ...
    const amount =
      typeof sum === "object" && sum && "amount" in sum
        ? Number(sum.amount)
        : 0;
