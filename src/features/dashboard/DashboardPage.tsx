...
export function DashboardPage({ onNavigate, onNewOrder }: { onNavigate: (key: ModuleKey) => void; onNewOrder: () => void }) {
  ...
  const activeOrderIds = realOrders.map((order) => String(order._id ?? "")).filter(Boolean).slice(0, 8);
  const latestLocationsQuery = useQuery({
    queryKey: ["dashboard-map-locations", activeOrderIds],
    queryFn: async () => {
      const results = await Promise.all(activeOrderIds.map((orderId) => tmsService.latestLocation(orderId).catch(() => null)));
  ...
  const mapLocations = mergeLocations(latestLocationsQuery.data ?? [], liveLocations);
  ...
  const stats = [
    { label: "Órdenes activas", value: String((ordersByStatus.REQUESTED ?? 0) + (ordersByStatus.ASSIGNED ?? 0) + (ordersByStatus.IN_PROGRESS ?? 0)), helper: summaryQuery.data ? "Datos reales API" : "Sin datos API", tone: "blue" as const, icon: PackageCheck },
    ...
    { label: "Conductores disponibles", value: String(summaryQuery.data?.activeDrivers ?? 0), helper: `${summaryQuery.data?.drivers ?? 0} conductores totales`, tone: "blue" as const, icon: Users },
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <section className="dashboard-primary">
        <article className="panel map-panel"><div className="panel-heading"><div><h2>Mapa en vivo</h2><p>Santo Domingo · rutas activas y zonas de incidencia</p></div><button className="text-link" onClick={() => onNavigate("map")}>Abrir mapa <ArrowRight size={14} /></button></div><LiveMap compact locations={mapLocations} /></article>
...
function incidentOrderLabel(incident: Record<string, unknown>) {
  ...
  if (order && typeof order === "object" && !Array.isArray(order) && "orderCode" in order) {
